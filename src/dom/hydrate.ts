...
// ---------------------------------------------------------------------------
// Legacy hydration API (preserved for backward compat)
// ---------------------------------------------------------------------------

let _isHydrating = false;
let _hydratingRoot: Element | null = null;
let _nodeIndex = 0;

/** Check if we're currently in hydration mode. */
export function isHydratingNow(): boolean {
  return _isHydrating;
}

/**
 * Hydrate server-rendered HTML by binding events and reactive effects
 * to existing DOM nodes.
 */
export function hydrate(root: Element, componentFn: () => Node): void {
  _isHydrating = true;
  _hydratingRoot = root;
  _nodeIndex = 0;

  try {
    // Collect all hydration-marked elements
    const markedElements = root.querySelectorAll('[data-forma-h]');
    const elementMap = new Map<number, Element>();
    for (const el of Array.from(markedElements)) {
      const id = parseInt(el.getAttribute('data-forma-h') ?? '-1', 10);
      if (id >= 0) elementMap.set(id, el);
    }

    // Collect all text markers
    const textMarkers = new Map<number, { start: Comment; end: Comment; textNode: Text | null }>();
    const walker = document.createTreeWalker(root, NodeFilter.SHOW_COMMENT);
    while (walker.nextNode()) {
      const comment = walker.currentNode as Comment;
      const startMatch = comment.data.match(/^forma-t:(\d+)$/);
      if (startMatch) {
        ...
        while (next) {
          if (next.nodeType === Node.COMMENT_NODE && (next as Comment).data === `/forma-t:${id}`) {
            endMarker = next as Comment;
            break;
          }
        ...
        if (endMarker) {
          textMarkers.set(id, { start: comment, end: endMarker, textNode });
        }
      }
    }
    ...
    _currentHydrationContext = hydrationCtx;
    componentFn();

  } finally {
    _isHydrating = false;
    _hydratingRoot = null;
    _currentHydrationContext = null;
    _nodeIndex = 0;
  }
}

// ---------------------------------------------------------------------------
// Hydration context (legacy)
// ---------------------------------------------------------------------------

interface HydrationContext {
  elementMap: Map<number, Element>;
  textMarkers: Map<number, { start: Comment; end: Comment; textNode: Text | null }>;
  nextElementId: number;
  nextTextId: number;
}

let _currentHydrationContext: HydrationContext | null = null;

export function getHydrationElement(): Element | null {
  if (!_currentHydrationContext) return null;
  const id = _currentHydrationContext.nextElementId++;
  return _currentHydrationContext.elementMap.get(id) ?? null;
}

export function getHydrationTextNode(): Text | null {
  if (!_currentHydrationContext) return null;
  const id = _currentHydrationContext.nextTextId++;
  const entry = _currentHydrationContext.textMarkers.get(id);
  return entry?.textNode ?? null;
}

export function hydrateEffect(el: Element, key: string, valueFn: () => unknown): void {
  internalEffect(() => {
    const val = valueFn();
    if (key === 'class' || key === 'className') {
      if (el instanceof HTMLElement) el.className = String(val ?? '');
      else el.setAttribute('class', String(val ?? ''));
    } else if (key === 'style') {
      (el as HTMLElement).style.cssText = String(val ?? '');
...
export function hydrateText(textNode: Text, valueFn: () => unknown): void {
  internalEffect(() => {
    const v = valueFn();
    textNode.data = (typeof v === 'string') ? v : String(v ?? '');
  });
}
